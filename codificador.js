...
botao.addEventListener("click", function (event) {
  ...
  var selection = document.querySelector("#tipo_cripto").value;
  if(radio[0].checked && selection == "cifra_de_cesar"){ // radio de codificar
...
function codifica(texto,chave){ // bruno,2
  return texto.map((teste)=>{
    ...
    if(cripto >= 65 && cripto<=90){  // alfabeto minúsculo
...
function decodifica(texto,chave){
  return texto.map((teste)=>{
    ...
    if(cripto >= 65 && cripto<=90){
      ...
      return String.fromCharCode(((cripto-90 - chave +26)%26)+90)  
     
    }else if(cripto >= 97 && cripto <= 122){
      
      return String.fromCharCode(((cripto-122 - chave +26)%26)+122)  
